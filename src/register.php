...
class Register{
    ...
    public function register($invokDesc, $invoker){
        ...
        $invoker->setHost($invoker->genDubboUrl($providerHost,$invokDesc,$this->config['protocol']));
    ...
    public function methodChangeHandler($invokerDesc, $provider){
        ...
        if($invokerDesc->isMatch($group,$version))
        {
            if($this->config['protocol']=='jsonrpc'){
                $host = 'http://'.$schemeInfo['host'].':'.$schemeInfo['port'];
            }else if($this->config['protocol']=='hessian'){
                $host = $schemeInfo['host'].':'.$schemeInfo['port'];
            }
            $this->providersCluster->addProvider($invokerDesc,$host,$schemeInfo['scheme']);
        }
    }
